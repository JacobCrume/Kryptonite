...
class DeviceConfig:
    def __init__(self, device):
        ...
        if 'device_private_key_filename' in device:
            self.device_private_key_filename = device['device_private_key_filename']
        # else:
        #     self.device_private_key_filename = os.path.join(os.path.dirname(__file__), FILES_FOLDER, device['name'], 'private_key.pem')

        if 'device_client_id_blob_filename' in device:
            self.device_client_id_blob_filename = device['device_client_id_blob_filename']
        # else:
        #     self.device_client_id_blob_filename = os.path.join(os.path.dirname(__file__), FILES_FOLDER, device['name'], 'client_id.bin')
